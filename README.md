...
*   **Отправка отдельных файлов в существующий проект**: Загружает один или несколько указанных файлов из локальной папки напрямую в существующую ветку проекта на GitLab через Commits API, не удаляя и не пересоздавая проект.
...
```bash
reposqueeze create-from-gitlab --repo-path /path/to/new/local/repo --branch-name gh-pages
```
...
### Отправка отдельных файлов в существующий проект

Эта команда отправляет указанные файлы из локальной папки в существующую ветку проекта на GitLab без удаления проекта.

```bash
reposqueeze push-files --repo-path /path/to/local/repo --branch-name master --files README.md,config.yaml
```

*   `--repo-path <путь_к_репозиторию>`: **(Обязательно)** Путь к локальному каталогу с файлами. Имя проекта на GitLab определяется автоматически по имени этой папки.
*   `--branch-name <имя_ветки>`: **(Обязательно)** Целевая ветка на GitLab (например, `master` или `main`).
*   `--files <список_файлов>`: **(Обязательно)** Список относительных путей к файлам через запятую.

**Пример:**
```bash
# Отправка одного файла
reposqueeze push-files --repo-path . --branch-name main --files README.md

# Отправка нескольких файлов
reposqueeze push-files --repo-path /home/user/my-project --branch-name develop --files src/main.go,go.mod,go.sum
```

### Скачивание файлов из GitLab

Эта команда скачивает файлы из существующего проекта на GitLab.

*   Если указан `--files` — забирает конкретные файлы.
*   Если `--files` не указан, а `--commits` равен 1 (по умолчанию) — определяет последний коммит в ветке и скачивает только изменённые в нём файлы (diff).
*   Если `--commits` больше 1 — обрабатывает указанное количество последних коммитов от старого к новому, скачивая файлы в состоянии каждого коммита. Удалённые файлы при этом удаляются локально.
*   Флаг `--git-add` добавляет все скачанные файлы в индекс локального git (`git add .`).

```bash
reposqueeze pull-files --repo-path /path/to/local/repo --branch-name master --files README.md
```

*   `--repo-path <путь_к_репозиторию>`: **(Обязательно)** Путь к локальному каталогу, куда сохранять файлы. Имя проекта на GitLab определяется автоматически по имени папки.
*   `--branch-name <имя_ветки>`: **(Опционально)** Исходная ветка на GitLab. По умолчанию `master`.
*   `--files <список_файлов>`: **(Опционально)** Список относительных путей к файлам через запятую. Если указан, `--commits` игнорируется.
*   `--commits <N>`: **(Опционально)** Количество последних коммитов для обработки (default: 1).
*   `--git-add`: **(Опционально)** После скачивания выполнить `git add .` и `git commit` в локальном репозитории.

**Пример:**
```bash
# Скачать конкретные файлы
reposqueeze pull-files --repo-path . --branch-name main --files README.md,go.mod

# Скачать файлы из последнего коммита (diff)
reposqueeze pull-files --repo-path /home/user/my-project --branch-name develop

# Скачать файлы из 3 последних коммитов и закоммитить локально
reposqueeze pull-files --repo-path . --branch-name develop --commits 3 --git-add
```
...
│   │       ├── push_files.go     # Логика отправки отдельных файлов в существующий проект
│   │       └── pull_files.go     # Логика скачивания файлов из GitLab
│   ├── domain/
│   │   ├── entity/
