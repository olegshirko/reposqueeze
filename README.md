...
Эта команда скачивает файлы из существующего проекта на GitLab. Если указан `--files` — забирает конкретные файлы. Если нет — определяет последний коммит в ветке и скачивает только те файлы, которые были изменены в нём (diff).
...
*   `--files <список_файлов>`: **(Опционально)** Список относительных путей к файлам через запятую. Если не указан, подтягиваются файлы из diff последнего коммита.
...
# Скачать все файлы из последнего коммита (diff)
